...
set -x
peer channel fetch 0 $BLOCKFILE -o orderer.supplychain.com:7050 --ordererTLSHostnameOverride orderer.supplychain.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
